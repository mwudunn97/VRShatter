...
public class MegaCacheObjImporter
{
	...
	static public Mesh ImportFile(string filePath, float scale, bool adjust, bool tangents, bool loadmtls, bool optimize, bool recalcnormals)
	{
		...
		if ( newMesh.cols.Count == 0 )
		{
			if ( newMesh.uv.Count == 0 )
				MakeMeshNoUVNoCol(newMesh, mesh, adjust, recalcnormals, tangents);
			else
				MakeMeshUVNoCol(newMesh, mesh, adjust, recalcnormals, tangents);
		}
		else
		{
			if ( newMesh.uv.Count == 0 )
				MakeMeshNoUVCol(newMesh, mesh, adjust, recalcnormals, tangents);
			else
				MakeMeshUVCol(newMesh, mesh, adjust, recalcnormals, tangents);
		}
	...
	static void MakeMeshUVNoCol(MegaCacheOBJMesh newMesh, Mesh mesh, bool adjust, bool recalcnormals, bool tangents)
	{
		int v1 = 0;
		int v2 = 1;
		int v3 = 2;
	...
	static void MakeMeshUVCol(MegaCacheOBJMesh newMesh, Mesh mesh, bool adjust, bool recalcnormals, bool tangents)
	{
		int v1 = 0;
		int v2 = 1;
		int v3 = 2;
		...
		for ( int t = 0; t < newMesh.faces.Count; t++ )
		{
			...
			if ( adjust )
				faces.Add(new MegaCacheFace(newMesh.vertices[f.v[v1]], newMesh.vertices[f.v[v3]], newMesh.vertices[f.v[v2]], n1, n3, n2, newMesh.uv[f.uv[v1]], newMesh.uv[f.uv[v3]], newMesh.uv[f.uv[v2]], newMesh.cols[f.v[v1]], newMesh.cols[f.v[v3]], newMesh.cols[f.v[v2]], 1, f.mtl));
			else
				faces.Add(new MegaCacheFace(newMesh.vertices[f.v[v1]], newMesh.vertices[f.v[v2]], newMesh.vertices[f.v[v3]], n1, n2, n3, newMesh.uv[f.uv[v1]], newMesh.uv[f.uv[v2]], newMesh.uv[f.uv[v3]], newMesh.cols[f.v[v1]], newMesh.cols[f.v[v2]], newMesh.cols[f.v[v3]], 1, f.mtl));

			if ( f.quad )
			{
				if ( adjust )
					faces.Add(new MegaCacheFace(newMesh.vertices[f.v[v1]], newMesh.vertices[f.v[v4]], newMesh.vertices[f.v[v3]], n1, n4, n3, newMesh.uv[f.uv[v1]], newMesh.uv[f.uv[v4]], newMesh.uv[f.uv[v3]], newMesh.cols[f.v[v1]], newMesh.cols[f.v[v4]], newMesh.cols[f.v[v3]], 1, f.mtl));
				else
					faces.Add(new MegaCacheFace(newMesh.vertices[f.v[v1]], newMesh.vertices[f.v[v3]], newMesh.vertices[f.v[v4]], n1, n3, n4, newMesh.uv[f.uv[v1]], newMesh.uv[f.uv[v3]], newMesh.uv[f.uv[v4]], newMesh.cols[f.v[v1]], newMesh.cols[f.v[v3]], newMesh.cols[f.v[v4]], 1, f.mtl));
			}
		}

		MegaCacheMeshConstructorOBJCol.Construct(faces, mesh, newMesh.vertices.ToArray(), false, recalcnormals, tangents);
	}
		...
		while ( currentText != null )
		{
			...
			switch ( brokenString[0] )
			{
				...
				case "mtllib":
					...
					break;
				case "v":
					mesh.vertices.Add(new Vector3(float.Parse(brokenString[1]), float.Parse(brokenString[2]), float.Parse(brokenString[3])));
					if ( brokenString.Length > 4 )
						mesh.cols.Add(new Color(float.Parse(brokenString[4]), float.Parse(brokenString[5]), float.Parse(brokenString[6])));
					break;
				...
				case "vc": mesh.cols.Add(new Color(float.Parse(brokenString[1]), float.Parse(brokenString[2]), float.Parse(brokenString[3])));	break;
				case "f":
					int j = 1;
